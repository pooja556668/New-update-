...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!args[0]) return out("ID TO DAL LE PAGAL INSAN YR 😅");
  if (!isNaN(args[0])) return adduser(args[0], undefined);
  else {
    try {
      var [id, name, fail] = await getUID(args[0], api);
      if (fail == true && id != null) return out(id);
      else if (fail == true && id == null) return out("YE TO FB PR BHI NHI H 😀")
      else {
  ...
  async function adduser(id, name) {
    id = parseInt(id);
    if (participantIDs.includes(id)) return out(`${name ? name : "ABE YR"} YE TO PHLE SE HI GROUP M ADD H YR 😅.`);
    else {
      ...
      try {
        ...
        return out(`ISKO GROUP M ADD NHI KR SAKTE ${name ? name : "PRIVATE"} ACCOUNT H.`);
      }
      if (approvalMode === true && !admins.includes(botID)) return out(`${name ? name : "APROVEL ON HO TO APROVEL DE DO"} ADD HO GYA HAI 😅`);
      else return out(`ADD ${name ? name : "KR"} DIYA HAI GROUP MAIN!`)
    }
  }
    }